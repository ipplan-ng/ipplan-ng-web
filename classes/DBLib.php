...
// open a database connection
	...
	if (DBF_TYPE=='mysqli' && !extension_loaded('mysqli'))
	{
   	die ('mysqli extension is not loaded');
	}

   	if (DBF_TYPE=='postgres9' && !extension_loaded('pgsql'))
	{
   	die ('pgsql extension is not loaded');
	}

	// create a connection id
        $this->ds = newAdoConnection(DBF_TYPE);
	    ...
	    {
		die ('Could not connect to database '.DBF_NAME);
	    }
        }
    }
        ...
        if ($this->ds->Affected_Rows() == 0) {
            $result = $this->ds->Execute("INSERT INTO ipaddr
                    ...
                    (".$this->ds->qstr("").",
                     ".$this->ds->qstr("").",
                     ".$this->ds->qstr("").",
                     ".$this->ds->qstr("Unknown - added by IPplan-NG poller").",
                     ".$this->ds->qstr("").",
                     $baseindex, 
                     $ipaddr,
                     ".$this->ds->DBTimeStamp(time()).",
